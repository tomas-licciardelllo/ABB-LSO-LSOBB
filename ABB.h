...
int baja_ABB(ABB *arbol,int DNI,prestador x)
{
    ...
    if(!localizar_ABB(*arbol, DNI,&aux,&padre_aux,0) /*||
        ...
        (strcmp(aux->info.servicios, x.servicios) != 0)*/) //Lo devolvemos al main al toque si no lo encuentra
    return 0;

    ///Caso sin hijos
    else{
        if((aux->info.DNI == x.DNI) &&
        (strcmp(aux->info.celular, x.celular) == 0) &&
        (strcmp(aux->info.domicilio, x.domicilio) == 0) &&
        (strcmp(aux->info.email, x.email) == 0) &&
        (strcmp(aux->info.nom_y_ape, x.nom_y_ape) == 0) &&
        (strcmp(aux->info.servicios, x.servicios) == 0))
        {
        ...
        {
            return -1;//si no es igual en todos los parametros lo devolvemos porque es distinto
        }
    }
    costosBajaABB[2] += 1;
...
prestador evocar_ABB(ABB arbol, int x){
    Nodo *aux;
    Nodo *papini;
     prestador aux_prestador = {0,"x","x","x","x","x"};
...
int mostrar_ABB_con_paginacion(ABB arbol, int nodos_por_pagina) {
    ...
    while (p.raiz != NULL) {
        ...
        Nodo *derecho = aux->pd;
        Nodo *izquierdo = aux->pi;
        // Mostrar informaci�n de los hijos izquierdo y derecho
        if (izquierdo != NULL) {
            printf("Identificador del nodo izquierdo DNI: %d \n", izquierdo->info.DNI);
        } else {
            printf("Nodo izquierdo: NULL\n");
        }

        if (derecho != NULL) {
            printf("Identificador del nodo derecho DNI: %d \n", derecho->info.DNI);
        } else {
